...
def EandI_classification(f_path):
    ...
    plt.ylabel('Count')
    plt.title('Count of E and I_The Picture of Dorian Gray')
    plt.show()
...
def NandS_classification(f_path):
    ...
    plt.bar(labels, counts, color=['skyblue', 'orange'])
    plt.xlabel('Personality Types_The Picture of Dorian Gray')
    plt.ylabel('Count')
...
path_list=[
    ...
    "C:\\Users\\jordi\\OneDrive\\바탕 화면\\23-2 연구\\data_set\\character_line\\The_Picture_of_Dorian_Gray\\ThePictureOfDorianGray_LordHenry_ENTP.csv"
]
